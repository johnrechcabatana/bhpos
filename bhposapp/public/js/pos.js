...
class POSItems {
	...
	get_item_this(item_code,warehouse){
		var company = "";
		var getQty = "";
			frappe.call({
				method: "bhposapp.public.py.test.get_query",
				freeze: true,
				args: {
					"item_code":item_code,
					"warehouse":warehouse
	...
	get_item_html(item) {
		...
		const { item_code, item_name, item_image, actual_qty, end_of_life} = item;
